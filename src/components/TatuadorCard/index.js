  ...
  render() {
    const results = data =>
      Object.keys(data).reduce((result, key) => {
        !~result.indexOf(data[key].estado) && result.push(data[key].estado)
        return result
      }, [])
      ...
      <>
        ...
        <div className="flex flex-wrap mb-4 md:w-1/1">
          {this.state.list
            ...
            .map(tatuador => (
              <div
                className="w-full md:w-1/2 lg:w-1/4 mt-8 p-2"
                key={tatuador.id}
              >
                  ...
                  <div className="sm:items-center px-6 py-4">
                    <div className="mt-4 sm:mt-0 sm:ml-4 text-center sm:text-left">
                      ...
                      <p className="text-sm leading-tight text-gray-600">
                        <strong>{tatuador.estado}</strong> - {tatuador.cidade}
                      </p>
                      <span className="text-sm leading-tight text-gray-600">
                        WhatsApp: {tatuador.numero}
                      </span>
                      <div className="flex mt-2 justify-center sm:justify-start ">
                        {tatuador.instagram ? (
                          <a
                            href={`https://www.instagram.com/${tatuador.instagram}`}
                            target="_blank"
                          >
                        ...
                        {tatuador.facebook ? (
                          <a
                            href={`https://www.facebook.com/search/top/?q=${tatuador.facebook}`}
                            target="_blank"
                          >
